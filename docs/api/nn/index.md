...
and process models; `phydrax.nn.layers` contains reusable tensor layers;
`phydrax.nn.parameters` contains physical parameter transforms and explicit
model-PyTree selections; and `phydrax.nn.operator` contains neural-operator data,
...
!!! note
    ...
    - Finite-dimensional sequence models share `RecurrentBatch` padding, reset,
      physical-time, and streaming-state semantics.
    - Operator tensor fields declare variance, parity, multiplicity, and finite-group
      actions explicitly; generic channels do not silently become physical tensors.
