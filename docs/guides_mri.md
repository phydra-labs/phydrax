...
`CartesianMRIEncodingPlan` applies coil sensitivities and orthonormal FFT sampling with an exact adjoint. `NonuniformMRIEncodingPlan` uses matched exact Type-2 and Type-1 nonuniform Fourier actions with bounded chunking. Density compensation is not part of the adjoint.
