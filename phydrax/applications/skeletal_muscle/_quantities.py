...
_AMPERE_PER_METER = derived_unit("A/m", ((AMPERE, 1), (METER, -1)))
_SIEMENS_PER_METER = derived_unit("S/m", ((SIEMENS, 1), (METER, -1)))
_JOULE_PER_CUBIC_METER = derived_unit("J/m3", ((JOULE, 1), (METER, -3)))
_WATT_PER_CUBIC_METER = derived_unit("W/m3", ((_WATT, 1), (METER, -3)))
_THERMAL_CONDUCTIVITY = derived_unit("W/(m*K)", ((_WATT, 1), (METER, -1), (KELVIN, -1)))
_VOLUMETRIC_HEAT_CAPACITY = derived_unit(
...
_REFERENCE_UNIT_BY_KIND = MappingProxyType(
    {
        ...
        "dimensionless": ONE,
        "electric_conductivity": _SIEMENS_PER_METER,
        "electric_current": AMPERE,
        "electric_potential": VOLT,
        "energy": JOULE,
        "energy_density": _JOULE_PER_CUBIC_METER,
        "force": _NEWTON,
        "frequency": _HERTZ,
        "length": METER,
        "line_current_density": _AMPERE_PER_METER,
        "power": _WATT,
        "pressure": PASCAL,
        "temperature": KELVIN,
        "thermal_conductivity": _THERMAL_CONDUCTIVITY,
        "volumetric_heat_capacity": _VOLUMETRIC_HEAT_CAPACITY,
        "volumetric_power_density": _WATT_PER_CUBIC_METER,
        "rate": _PER_SECOND,
...
_UNIT_BY_SYMBOL = MappingProxyType(
    {
        ...
        "A/m": _AMPERE_PER_METER,
        "1/s": _PER_SECOND,
        "A": AMPERE,
        "Hz": _HERTZ,
        "J": JOULE,
        "J/m3": _JOULE_PER_CUBIC_METER,
        "J/(m3*K)": _VOLUMETRIC_HEAT_CAPACITY,
        "K": KELVIN,
        "N": _NEWTON,
        "Pa": PASCAL,
        "S/m": _SIEMENS_PER_METER,
        "V": VOLT,
        "W": _WATT,
        "m": METER,
        "m/s": _METER_PER_SECOND,
        "ms": MILLISECOND,
        "W/m3": _WATT_PER_CUBIC_METER,
        "W/(m*K)": _THERMAL_CONDUCTIVITY,
...
_SPECS = (
    ...
    _quantity(
        ...
        "dimensionless",
        "1",
        sign="source-defined finite amplitude; GASAM plans enforce [0, 1]",
        support="skeletal continuum material point",
        reference="owning source-named plan; Almonacid executable controls are not clipped",
    ),
    _quantity(
    ...
    _quantity(
        ...
        "A",
        axes=("fiber", "node"),
        sign="positive outward from the fiber",
        support="sealed-end fiber nodal control-volume integrated membrane current",
        reference="Pereira Botelho et al. 2019 current geometry; not ionic current alone",
    ),
    _quantity(
        ...
        axes=("fiber", "node"),
        sign="positive outward from the fiber",
        support="sealed-end fiber membrane current per unit current arc length",
        reference="Pereira Botelho et al. 2019 current geometry",
    ),
    _quantity(
        ...
        "S/m",
        axes=("fiber",),
        sign="strictly positive longitudinal conductivity",
        support="intracellular sealed-end fiber cable",
        reference="bound cable capacitance, diffusivity and radius; 2 C_m D / r",
    ),
    _quantity(
        ...
        "V",
        axes=("contact",),
        sign="signed rectangular electrode-area average",
        support="passive cylindrical skin surface contacts",
        reference="Farina et al. 2004 cylindrical conductor; zero axial-average gauge",
    ),
    _quantity(
        ...
        "V",
        axes=("lead",),
        sign="signed gauge-neutral lead montage",
        support="instantaneous cylindrical surface electrode leads",
        reference="Farina et al. 2004 contact-potential differences",
    ),
    _quantity(
        "skeletal_continuum_first_piola_stress",
        "pressure",
        "Pa",
        axes=("spatial_i", "reference_J"),
        sign="tension-positive normal traction; pressure-multiplier sign belongs to the source-named material",
        support="skeletal continuum material point or finite-element quadrature point",
        reference="two-point current/reference tensor; sole continuum force owner",
    ),
    _quantity(
        "skeletal_continuum_second_piola_stress",
        "pressure",
        "Pa",
        axes=("reference_I", "reference_J"),
        sign="tension-positive normal stress",
        support="skeletal continuum material point or finite-element quadrature point",
        reference="reference-configuration tensor; sole continuum force owner",
    ),
    _quantity(
        "skeletal_continuum_cauchy_stress",
        "pressure",
        "Pa",
        axes=("spatial_i", "spatial_j"),
        sign="tension-positive normal stress",
        support="skeletal continuum material point or finite-element quadrature point",
        reference="current-configuration tensor; sole continuum force owner",
    ),
    _quantity(
        "skeletal_continuum_passive_reference_energy_density",
        "energy_density",
        "J/m3",
        sign="stored passive energy relative to the undeformed reference",
        support="skeletal continuum material point or finite-element quadrature point",
        reference="energy per reference volume; excludes nonconservative active stress",
    ),
    _quantity(
        "skeletal_active_nominal_stress",
        "pressure",
        "Pa",
        sign="source-signed reference-fiber nominal stress; not clipped",
        support="skeletal continuum material point or finite-element quadrature point",
        reference="Heidlauf--Roehrle 2014 Pmax times prescribed normalized active stress",
    ),
    _quantity(
        "skeletal_prescribed_normalized_active_stress",
        "dimensionless",
        "1",
        sign="signed finite prescribed input; no activation bound [0, 1]",
        support="skeletal continuum material point or finite-element quadrature point",
        reference="Heidlauf--Roehrle 2014 gamma-bar; not a produced cellular stress",
    ),
    _quantity(
        "skeletal_continuum_volume_ratio",
        "dimensionless",
        "1",
        sign="strictly positive current volume divided by reference volume",
        support="skeletal continuum material point or finite-element quadrature point",
        reference="determinant of the material deformation gradient",
    ),
    _quantity(
        "skeletal_continuum_fiber_stretch",
        "dimensionless",
        "1",
        sign="strictly positive current source-fiber length",
        support="skeletal continuum material point or finite-element quadrature point",
        reference="norm of F a0; each source owner declares whether a0 is normalized",
    ),
    _quantity(
        "temperature",
        "temperature",
        "K",
        axes=("node",),
        sign="strictly positive absolute temperature",
        support="fixed reference finite-element mesh nodes",
        reference="Pennes 1948 scalar thermal field; kelvin scale",
    ),
    _quantity(
        ...
        "W/m3",
        axes=("cell",),
        sign="positive retained heat supplied to tissue",
        support="fixed reference finite-element cells",
        reference="Pennes 1948 interval source per reference volume; not total metabolism",
    ),
    _quantity(
        ...
        "W/m3",
        axes=("cell",),
        sign="positive heat supplied from blood to tissue",
        support="fixed reference finite-element cells",
        reference="Pennes 1948 blood volumetric capacity times perfusion times arterial-minus-tissue temperature",
    ),
    _quantity(
        ...
        "W/(m*K)",
        axes=("region",),
        sign="strictly positive isotropic scalar conductivity",
        support="fixed reference thermal tissue regions",
        reference="Pennes 1948 caller evidence bundle; no tensor or physiological default",
    ),
    _quantity(
        ...
        "J/(m3*K)",
        axes=("region",),
        sign="strictly positive heat capacity per reference volume",
        support="fixed reference thermal tissue regions",
        reference="Pennes 1948 caller evidence bundle",
    ),
)
