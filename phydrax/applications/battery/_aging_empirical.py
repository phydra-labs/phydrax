...
class EmpiricalAgingStatus(IntEnum):
    """Fail-closed disposition for one physical aging macrostep."""
...
def _support_bounds(
    ...
    *,
    ...
    return jax.lax.stop_gradient(bounds.astype(jnp.result_type(bounds, jnp.float64)))
...
class EmpiricalAgingSupport(StrictModule, NonTrainableState):
    ...
    def __init__(
        ...
        *,
        ...
        soc_host = np.asarray(state_of_charge, dtype=np.float64)
        if soc_host[1] > 1.0:
        ...
        source = _identifier(source_id, "Empirical aging support source ID")
        ...
        self.support_id = canonical_fingerprint(
            {
                "kind": "battery-empirical-aging-support",
                "temperature_bounds_k": np.asarray(
                    temperature, dtype=np.float64
                ).tolist(),
                "state_of_charge_bounds": np.asarray(
                    state_of_charge, dtype=np.float64
                ).tolist(),
                "current_magnitude_bounds_a": np.asarray(
...
class EmpiricalAgingTopology(StrictModule, NonTrainableState):
    """Fixed stress-history shape bound to an immutable calibration support."""

    support: EmpiricalAgingSupport
    ...
    def __init__(self, support: EmpiricalAgingSupport, history_node_count: int, /):
        ...
        if history_node_count < 2:
            raise ValueError("Empirical aging histories require at least two nodes.")
        self.support = support
        self.history_node_count = history_node_count
        self.topology_id = canonical_fingerprint(
            {
                "kind": "battery-empirical-aging-topology",
    ...
    def advance(
        self,
        coefficients: "EmpiricalAgingCoefficients",
        state: "EmpiricalAgingState",
        history: "EmpiricalAgingStressHistory",
        /,
    ) -> "EmpiricalAgingTransitionResult":
        return advance_empirical_aging(self, coefficients, state, history)


class EmpiricalAgingCoefficients(StrictModule):
    ...
    def __init__(
        ...
        calendar_rate = eqx.error_if(
            calendar_rate,
            ~valid,
            "Empirical aging coefficients are nonfinite or outside their physical domain.",
        )
...
class EmpiricalAgingState(StrictModule):
...
class EmpiricalAgingStressHistory(StrictModule):
    ...
    def __init__(
        ...
        if arrays[0].ndim != 1 or arrays[0].size < 2:
            raise ValueError(
                "Empirical aging stress histories require at least two nodes."
            )
        if any(value.shape != arrays[0].shape for value in arrays[1:]):
            raise ValueError(
                "Empirical aging stress histories must share one fixed shape."
            )
        if any(jnp.issubdtype(value.dtype, jnp.complexfloating) for value in arrays):
            raise TypeError("Empirical aging stress histories must be real-valued.")
...
class EmpiricalAgingStressSummary(StrictModule):
...
class EmpiricalAgingObservables(StrictModule):
...
class EmpiricalAgingTransitionResult(StrictModule):
    """Candidate, accepted, stress, and fail-closed status for one macrostep."""

    candidate_state: EmpiricalAgingState
    accepted_state: EmpiricalAgingState
    stress_summary: EmpiricalAgingStressSummary
    observables: EmpiricalAgingObservables
    status: Array

    @property
    def successful(self) -> Array:
        return self.status == int(EmpiricalAgingStatus.SUCCESS)


def initial_empirical_aging_state(
    ...
    *,
    ...
    return EmpiricalAgingState(zero, zero, time, throughput)


def empirical_aging_observables(
    coefficients: EmpiricalAgingCoefficients,
    state: EmpiricalAgingState,
    /,
) -> EmpiricalAgingObservables:
    """Evaluate the exact nonlinear law from its latent exposure state."""

    if not isinstance(coefficients, EmpiricalAgingCoefficients):
        raise TypeError("coefficients must be EmpiricalAgingCoefficients.")
    if not isinstance(state, EmpiricalAgingState):
        raise TypeError("state must be EmpiricalAgingState.")
    ...
    return EmpiricalAgingObservables(
        calendar_damage,
        throughput_damage,
        total_damage,
        capacity_loss,
...
def _accepted_state(
    successful: Array,
    candidate: EmpiricalAgingState,
    previous: EmpiricalAgingState,
    /,
) -> EmpiricalAgingState:
    return EmpiricalAgingState(
...
def advance_empirical_aging(
    topology: EmpiricalAgingTopology,
    coefficients: EmpiricalAgingCoefficients,
    state: EmpiricalAgingState,
    history: EmpiricalAgingStressHistory,
    /,
    ...
    if not isinstance(topology, EmpiricalAgingTopology):
        raise TypeError("topology must be EmpiricalAgingTopology.")
    if not isinstance(coefficients, EmpiricalAgingCoefficients):
        raise TypeError("coefficients must be EmpiricalAgingCoefficients.")
    if not isinstance(state, EmpiricalAgingState):
        raise TypeError("state must be EmpiricalAgingState.")
    if not isinstance(history, EmpiricalAgingStressHistory):
        raise TypeError("history must be EmpiricalAgingStressHistory.")
    if history.times_s.size != topology.history_node_count:
        raise ValueError("Stress history shape does not match the aging topology.")
    ...
    candidate_observables = empirical_aging_observables(coefficients, candidate)
    ...
    status = jnp.asarray(int(EmpiricalAgingStatus.SUCCESS), dtype=jnp.int32)
    status = jnp.where(~state_valid, int(EmpiricalAgingStatus.INVALID_STATE), status)
    status = jnp.where(
        (status == int(EmpiricalAgingStatus.SUCCESS)) & ~times_valid,
        int(EmpiricalAgingStatus.INVALID_TIMES),
        status,
    )
    status = jnp.where(
        (status == int(EmpiricalAgingStatus.SUCCESS)) & ~gaps_valid,
        int(EmpiricalAgingStatus.TIME_GAP_EXCEEDED),
        status,
    )
    status = jnp.where(
        (status == int(EmpiricalAgingStatus.SUCCESS)) & ~macrostep_valid,
        int(EmpiricalAgingStatus.MACROSTEP_EXCEEDED),
        status,
    )
    status = jnp.where(
        (status == int(EmpiricalAgingStatus.SUCCESS)) & ~temperature_valid,
        int(EmpiricalAgingStatus.TEMPERATURE_OUT_OF_SUPPORT),
        status,
    )
    status = jnp.where(
        (status == int(EmpiricalAgingStatus.SUCCESS)) & ~state_of_charge_valid,
        int(EmpiricalAgingStatus.STATE_OF_CHARGE_OUT_OF_SUPPORT),
        status,
    )
    status = jnp.where(
        (status == int(EmpiricalAgingStatus.SUCCESS)) & ~current_valid,
        int(EmpiricalAgingStatus.CURRENT_OUT_OF_SUPPORT),
        status,
    )
    status = jnp.where(
        (status == int(EmpiricalAgingStatus.SUCCESS)) & ~throughput_valid,
        int(EmpiricalAgingStatus.THROUGHPUT_OUT_OF_SUPPORT),
        status,
    )
    status = jnp.where(
        (status == int(EmpiricalAgingStatus.SUCCESS)) & ~transition_finite,
        int(EmpiricalAgingStatus.NONFINITE_TRANSITION),
        status,
    )
    successful = status == int(EmpiricalAgingStatus.SUCCESS)
    accepted = _accepted_state(successful, candidate, state)
    observables = empirical_aging_observables(coefficients, accepted)

    safe_duration = jnp.where(
        jnp.isfinite(duration) & (duration > 0.0), duration, jnp.ones((), dtype=dtype)
    )
    summary = EmpiricalAgingStressSummary(
        duration,
    ...
    return EmpiricalAgingTransitionResult(
        candidate,
        accepted,
        summary,
        observables,
...
__all__ = [
    "EmpiricalAgingCoefficients",
    "EmpiricalAgingObservables",
    "EmpiricalAgingState",
    "EmpiricalAgingStatus",
    "EmpiricalAgingStressHistory",
    "EmpiricalAgingStressSummary",
    "EmpiricalAgingSupport",
    "EmpiricalAgingTopology",
    "EmpiricalAgingTransitionResult",
    "advance_empirical_aging",
    "empirical_aging_observables",
