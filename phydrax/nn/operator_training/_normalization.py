...
from ..models.core._operator_field import OperatorFieldSpec
    ...
    for index, (value, mask) in enumerate(zip(arrays, masks, strict=True)):
        ...
        if weights is not None:
            ...
            if weight.shape != valid.shape:
                raise ValueError(
                    "Normalization weights must match case/sample geometry."
                )
            if np.any(~np.isfinite(weight)) or np.any(weight < 0.0):
                raise ValueError(
                    "Normalization weights must be finite and nonnegative."
                )
...
class OperatorNormalizationPolicy:
            ...
            if axes:
                ...
                for index, axis in enumerate(axes):
                    ...
                    if quadrature is not None:
                        quadrature = (
                            quadrature * scale if inverse else quadrature / scale
                        )
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "targets": {
                name: normalizer.to_dict()
                for name, normalizer in self.targets.items()
            },
            "input_coordinates": {
                name: normalizer.to_dict()
...
def fit_operator_normalization(
    ...
    *,
    ...
    target_tuple = (
        (targets,)
        if isinstance(targets, OperatorTargetBatch)
        else tuple(targets)
    )
    ...
    for field in field_specs:
        if field.is_source:
            ...
            if field.source_name in source_fields:
                raise ValueError("Operator source names must map to unique field semantics.")
            source_fields[field.source_name] = field
        if field.is_target:
            target_fields[field.name] = field

    configured_input_axes = (
        {} if input_channel_axes is None else dict(input_channel_axes)
    )
    ...
    for name in input_names:
        ...
        input_values[name] = _fit_arrays(
            ...
            center=not (
                source_cochain is not None
                and source_cochain.cell_orientation == "signed"
            ),
        )
    ...
    for name in target_names:
        ...
        target_normalizers[name] = _fit_arrays(
            ...
            weights=weights,
            center=not (
                target_cochain is not None
                and target_cochain.cell_orientation == "signed"
            ),
        )
