...
from ..models.core._operator_capabilities import OperatorTrainingEvidence
...
def _qualified_name(value: Any, /) -> str:
    ...
    if "<locals>" in qualname or "<lambda>" in qualname:
        raise TypeError("Portable operator artifacts cannot encode local callables.")
    root = module.split(".", 1)[0]
    if root not in _ALLOWED_MODULE_ROOTS:
        raise TypeError(f"Portable operator artifacts do not trust module root {root!r}.")
    return f"{module}:{qualname}"


def _resolve_qualified(value: str, /) -> Any:
    module_name, separator, qualname = value.partition(":")
    if not separator or module_name.split(".", 1)[0] not in _ALLOWED_MODULE_ROOTS:
        raise ValueError("Operator artifact contains an untrusted qualified name.")
    resolved: Any = importlib.import_module(module_name)
    for component in qualname.split("."):
        resolved = vars(resolved)[component]
    return resolved
...
def _structure_recipe(value: Any, /, *, path: str = "model") -> dict[str, Any]:
    ...
    if isinstance(value, type):
        return {"kind": "type", "value": _qualified_name(value)}
    ...
    if callable(value):
        return {"kind": "callable", "value": _qualified_name(value)}
    raise TypeError(
...
def _from_structure_recipe(recipe: Mapping[str, Any], /) -> Any:
    ...
    if kind == "ellipsis":
        return Ellipsis
    if kind == "enum":
        return _resolve_qualified(recipe["type"])[recipe["name"]]
    if kind in ("type", "callable"):
        return _resolve_qualified(recipe["value"])
    if kind == "namedtuple":
        cls = _resolve_qualified(recipe["type"])
...
class OperatorArtifactManifest:
    ...
    def from_dict(cls, value: Mapping[str, Any], /) -> "OperatorArtifactManifest":
        ...
        return cls(
            ...
            execution_model_portable=bool(value["execution_model_portable"]),
            execution_model_factory_id=str(value["execution_model_factory_id"]),
...
def save_operator_artifact(
    ...
    *,
    ...
    from ..models.wrappers._operator_adapter import ExternalOperatorAdapter
    ...
    for candidate in destination.iterdir():
        if candidate.is_file() and candidate.name not in keep:
            if candidate.name.startswith(
                ("model-", "execution-model-", "training-")
            ):
                candidate.unlink()
    return destination
...
def load_trained_operator(
    ...
    /,
    *,
    execution_model_like: _AbstractOperatorModel | None = None,
    ...
    if not isinstance(model_template, _AbstractOperatorModel):
        raise TypeError(
            "Operator artifact recipe did not restore an execution model."
        )
...
def _fixed_query_fingerprints(
    ...
    return {
        name: batch.query(name).geometry_fingerprint()
        for name in task.query_by_name
    }
...
def load_external_trained_operator(
    ...
    *,
    ...
    from ..models.wrappers._operator_adapter import load_external_operator_adapter
