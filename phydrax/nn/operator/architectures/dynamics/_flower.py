...
from phydrax.nn.layers._probabilistic_warp import ProbabilisticMultiheadWarp
from phydrax.nn.layers._warp import (
    _boundary_modes,
    MultiheadWarp,
    WarpBoundaryMode,
)
from phydrax.nn.layers._warp_geometry import (
...
class _ChannelLastGroupNorm(StrictModule):
    ...
    def __call__(
        ...
        *,
        ...
        reduction_axes = tuple(range(case_ndim, case_ndim + self.spatial_ndim)) + (
            grouped.ndim - 1,
        )
        if mask is None:
        ...
        if modulation is not None:
            ...
            if modulation_scale.shape != expected or modulation_shift.shape != expected:
                raise ValueError(
...
class _ResolutionConsistentTransitionND(StrictModule):
    ...
    def _downsample(
        ...
        for local_axis in range(self.spatial_ndim):
            ...
            denominator = left_mask_weight * left_valid + right_mask_weight * right_valid
            output = (
...
class Flower(AbstractOperatorModel):
    ...
    down_convolutions: tuple[_StrideTwoConvND | _ResolutionConsistentTransitionND, ...]
    bottleneck: _FlowerBlock
    decoder_blocks: tuple[_FlowerBlock, ...]
    up_convolutions: tuple[_StrideTwoConvND | _ResolutionConsistentTransitionND, ...]
    ...
    def _project_conservation(
        ...
        valid_target_weights = target_weights * target_mask.astype(target_weights.dtype)
        target_mass = jnp.sum(
        ...
        broadcast = (
            source_mass.shape[:-1] + (1,) * len(target_axes) + (source_mass.shape[-1],)
        )
        ...
        *,
        ...
        if self.transition_mode == "learned":
            ...
            for block, down in zip(
                ...
                skips.append((hidden, current_nodes, current_weights, current_mask))
    ...
    def _normalize_query_coordinates(
        ...
        if query.axes:
            if len(query.axes) != self.spatial_ndim:
                raise ValueError(f"Flower query grid must have {self.spatial_ndim} axes.")
        ...
        for index, source_axis in enumerate(source_axes):
            ...
            components.append(-1.0 + 2.0 * (physical[..., index] - nodes[0]) / scale)
    ...
    def _call_operator_batch(
        ...
        *,
        ...
        if not self.conserve_mass:
            query_weights = batch.require_single_query().quadrature(
    ...
    def __call__(
        ...
        *,
        ...
        if self.conditioning_channels:
            raise ValueError("Conditioned Flower evaluation requires an OperatorBatch.")
        if not isinstance(x, tuple) or len(x) != self.spatial_ndim + 1:
            raise ValueError(
                "Flower requires (values, axis_0, ..., axis_d) structured input."
            )
