...
class ArrayOperatorQuerySource(OperatorQuerySource):
    ...
    def __init__(
        ...
        *,
        ...
        self.coordinates = coordinates.reshape(cases + (size, coordinate_dimension))
    ...
    def read_chunk(self, start: int, size: int, /) -> OperatorQueryChunk:
        ...
        samples = FunctionSamples(
            ...
            quadrature_weights=_take_query_axis(self.quadrature, indices, case_ndim),
...
class ArrayPredictionSink(OperatorPredictionSink):
    ...
    def write(self, start: int, values: np.ndarray, /) -> None:
        ...
        if array.shape != expected:
            raise ValueError(
                f"Prediction chunk must have shape {expected}; got {array.shape}."
            )
        ...
        if self.resume:
            if not self.path.exists() or not self.metadata_path.exists():
                raise FileNotFoundError(
                    "Resumed prediction requires data and metadata files."
                )
            ...
            if recorded != metadata:
                raise ValueError(
                    "Resumed prediction metadata does not match the request."
                )
...
def decode_query_chunks(
    ...
    *,
    ...
    if not isinstance(model, OperatorModel):
        raise TypeError("decode_query_chunks requires an OperatorModel.")
    encoded = isinstance(model, EncodedOperatorModel)
    if encoded_state is not None and not encoded:
        raise TypeError("encoded_state requires an EncodedOperatorModel.")
    state = (
    ...
    if encoded:
        ...
        def evaluate(query: FunctionSamples, eval_key: EvalKey) -> Array:
            assert isinstance(encoded_model, EncodedOperatorModel)
            return encoded_model.decode_query(state, query, key=eval_key)

    else:

        def evaluate(query: FunctionSamples, eval_key: EvalKey) -> Array:
            query_batch = OperatorBatch(
                inputs=batch.inputs,
                queries={"query": query},
                case_axes=batch.case_axes,
                case_shape=batch.case_shape,
            )
            prediction = model.predict_prevalidated(query_batch, key=eval_key)
            if len(prediction.fields) != 1:
                raise ValueError(
                    "Streaming query decoding requires exactly one output field."
                )
            return next(iter(prediction.fields.values())).values
