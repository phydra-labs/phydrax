...
from ._operator_context import EncodedOperatorState
...
class OperatorBranchGraph(StrictModule):
    ...
    def __init__(
        ...
        *,
        ...
        self.branches = frozendict(
            {branch.name: branch for branch in branch_tuple}
        )
    ...
    def prediction_names(self) -> tuple[str, ...]:
        return tuple(
            name for name, branch in self.branches.items() if branch.predicts
        )

    @property
    def conditioning_names(self) -> tuple[str, ...]:
        return tuple(
            name for name, branch in self.branches.items() if branch.conditions
        )
...
def apply_branch_interactions(
    ...
    for name, incoming in updates.items():
        ...
        replacements[name] = target.replace_layers(
            target.layer_values + (updated,)
        )
