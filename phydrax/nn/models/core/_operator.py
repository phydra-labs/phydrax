...
class OperatorAxis(StrictModule):
    ...
    def from_discretization(
        cls,
        name: str,
        discretization: AxisDiscretization,
        /,
    ) -> "OperatorAxis":
        return cls(
            name,
...
class FunctionSamples(StrictModule):
    ...
    def geometry_fingerprint(self) -> str:
        ...
        for axis in self.axes:
            digest.update(
                repr((axis.name, axis.basis, axis.periodic)).encode("utf-8")
            )
...
def _validate_sample_values(
    ...
    *,
    ...
    if (
        sample_shape
        and tuple(
            int(size) for size in array.shape[case_ndim : case_ndim + sample_ndim]
        )
        != sample_shape
    ):
        raise ValueError(
    ...
    def take(self, index: Any, /, *, axis: int | str = 0) -> "OperatorTargetBatch":
        if isinstance(axis, str):
            if axis not in self.case_axes:
                raise KeyError(
                    f"Unknown case axis {axis!r}; expected {self.case_axes}."
                )
