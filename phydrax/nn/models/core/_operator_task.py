...
from ._operator import FunctionSamples, OperatorBatch, OperatorPrediction
...
from ._operator_topology import OperatorTopologySite
...
def _problem_from_dict(value: Mapping[str, Any], /) -> OperatorProblemSpec:
    ...
    return OperatorProblemSpec(
        ...
        requires_encode_once_decode_many=bool(
            value["requires_encode_once_decode_many"]
        ),
...
class OperatorQuerySpec(StrictModule):
    ...
    def __init__(
        ...
        *,
        ...
        if geometry_kind in ("graph", "simplicial", "cell_complex") and topology_site is None:
            raise ValueError("Topological queries require a topology site.")
        if geometry_kind not in ("graph", "simplicial", "cell_complex") and topology_site is not None:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "coordinate_dimensions": [list(value) for value in self.coordinate_dimensions],
...
class OperatorTask(StrictModule):
    ...
    def __init__(
        ...
        *,
        ...
        if not fields_ or any(not isinstance(field, OperatorFieldSpec) for field in fields_):
            raise TypeError("Operator tasks require at least one OperatorFieldSpec.")
        if not queries_ or any(not isinstance(query, OperatorQuerySpec) for query in queries_):
    ...
    def _validate_pde(
        ...
        for pde_field in pde.fields:
            ...
            if expected_representation is None or field.representation != expected_representation:
                raise ValueError(
    ...
    def validate_batch(self, batch: OperatorBatch, /) -> None:
        ...
        for query_spec in self.queries:
            ...
            if (
                query_spec.topology_site is not None
                and (
    ...
    def validate_prediction(self, prediction: OperatorPrediction, /) -> None:
        ...
        if not isinstance(prediction, OperatorPrediction):
            raise TypeError("OperatorTask.validate_prediction requires an OperatorPrediction.")
        expected_queries = tuple(query.name for query in self.queries)
