...
def _sample_regular_grid_linear(
    ...
    *,
    ...
    if jnp.issubdtype(array.dtype, jnp.complexfloating):
        raise TypeError("Regular-grid warping currently supports real-valued arrays only.")
    ...
    spatial_shape = tuple(
        int(size) for size in array.shape[-spatial_ndim - 1 : -1]
    )
    ...
    for axis, (size, mode) in enumerate(zip(spatial_shape, boundary, strict=True)):
        ...
        if mode == "periodic":
            ...
            if mode == "reflect":
                ...
                outside = outside | (continuous < 0.0) | (
                    continuous > float(size - 1)
                )
                continuous = jnp.clip(continuous, 0.0, float(size - 1))
            lower_float = jnp.floor(continuous)
            lower = lower_float.astype(jnp.int32)
...
class MultiheadWarp(StrictModule):
    ...
    def __init__(
        ...
        *,
        ...
        hidden_width = (
            self.out_channels
            if displacement_width is None
            else int(displacement_width)
        )
        if hidden_width <= 0:
            raise ValueError("displacement_width must be positive.")
        if self.mask_mode not in ("reject", "renormalize", "strict"):
            raise ValueError(
                "mask_mode must be 'reject', 'renormalize', or 'strict'."
            )
        ...
        *,
        ...
        if self.displacement_condition is None:
            if condition is not None:
                raise ValueError(
                    "condition must be None when conditioning_size is zero."
                )
            return hidden
    ...
    def displacement(
        ...
        *,
        ...
        return self.displacement_output(jax.nn.gelu(hidden)).reshape(
            case_shape
            + spatial_shape
            + (self.num_heads, self.spatial_ndim)
        )
