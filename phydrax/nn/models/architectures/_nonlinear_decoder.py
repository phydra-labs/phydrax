    ...
    def __init__(
        ...
        *,
        ...
        if min(self.latent_size, self.coord_dim, self.width, self.depth) <= 0:
            raise ValueError(
                "latent_size, coord_dim, width, and depth must be positive."
            )
    ...
    def __init__(
        ...
        *,
        ...
        if _get_size(decoder.in_size) != self.latent_size + self.coord_dim:
            raise ValueError(
                "Decoder input size must equal latent_size + coord_dim."
            )
    ...
    def encode_inputs(
        ...
        *,
        ...
        if self.fusion == "sum":
            ...
            latent = jax.vmap(
                lambda value: branch_mixer(value, key=keys[-1])
            )(flattened).reshape(batch.case_shape + (self.latent_size,))
