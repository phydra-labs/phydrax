...
from ..layers._warp import _boundary_modes, WarpBoundaryMode
from ..layers._warp_geometry import WarpMaskMode
from ._flower import Flower, FlowerTransitionMode
from ._geometry_operator import (
...
class GeometryInformedFlower(_AbstractOperatorModel):
    ...
    def __init__(
        ...
        *,
        ...
        if support_key is not None and source_mask_mode == "reject":
            raise ValueError(
                "Hard latent support requires source_mask_mode='strict' or "
                "'renormalize'."
            )
        threshold = (
            0.5 if latent_support_kind == "occupancy" else 0.0
        ) if latent_support_threshold is None else float(latent_support_threshold)
