...
from ..core._operator_prompt import OperatorPrompt, PromptedOperatorBatch
...
class InContextOperator(AbstractEncodedOperatorModel):
    ...
    def __init__(
        ...
        *,
        ...
        self.current_attention = MeasureAwareAttention(
            key=keys[7], **attention_kwargs
        )
        self.context_attention = MeasureAwareAttention(
            key=keys[8], **attention_kwargs
        )
    ...
    def _source(self, batch: OperatorBatch, /) -> FunctionSamples:
        ...
        if len(batch.inputs) != 1:
            raise ValueError(
                "InContextOperator requires source_key for multiple inputs."
            )
    ...
    def encode_with_prompt_state(
        ...
        memory = prompt_state.values.reshape(
            (cases, prompt_state.num_tokens, self.width)
        )
