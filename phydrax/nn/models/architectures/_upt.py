...
def _flatten_function_values(
    ...
    if tuple(int(size) for size in array.shape) == scalar_shape:
        ...
        and tuple(int(size) for size in array.shape[len(case_shape) : -1])
        == sample_shape
    ):
...
class LatentTokenBlock(StrictModule):
    ...
    def __call__(
        ...
        feed_forward = self.output(jnn.silu(self.gate(normalized)) * self.value(normalized))
...
class UPT(AbstractEncodedOperatorModel):
    ...
    def _source(self, batch: OperatorBatch, /) -> FunctionSamples:
        ...
        if len(batch.inputs) != 1:
            raise ValueError("UPT requires source_key when OperatorBatch has multiple inputs.")
    ...
    def encode_inputs(
        ...
        *,
        ...
        source_features = self.source_lift(jnp.concatenate((values, coordinates), axis=-1))
    ...
    def __init__(
        ...
        *,
        ...
        if set(input_channels) != set(self.conditioning_names):
            raise ValueError("input_channels must cover every conditioning branch exactly.")
        ...
        if set(resolved_anchors) != set(self.conditioning_names):
            raise ValueError("anchor_counts must cover every conditioning branch exactly.")
        ...
        self.in_size = tuple(self.input_channels[name] for name in self.conditioning_names)
