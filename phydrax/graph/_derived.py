...
def _edge_graph_ids(graph: GraphIR, /) -> np.ndarray:
    graph_ids = np.arange(graph.n_edge.shape[0], dtype=np.int32)
...
def line_graph(
    ...
    *,
    ...
    if source_edges.shape[0] == 0:
        n_line_edge = np.zeros((graph.n_edge.shape[0],), dtype=np.int32)
    else:
        n_line_edge = np.bincount(
            edge_graph_ids[source_edges],
            minlength=graph.n_edge.shape[0],
        ).astype(np.int32)
    edges = {
    ...
    return LineGraph(
        line,
        original_edges=jnp.arange(graph.senders.shape[0], dtype=jnp.int32),
        transition_edges=jnp.arange(source_edges.shape[0], dtype=jnp.int32),
...
class MeshDualGraph(StrictModule):
...
def mesh_to_dual_graph(
    ...
    *,
    ...
    if "features" in nodes:
        ...
        if features.ndim == 0 or features.shape[0] != n_face:
    ...
    nodes["centroid"] = jnp.asarray(centroid, dtype=jnp.float64)
    nodes["normal"] = jnp.asarray(normal, dtype=jnp.float64)
    nodes["area"] = jnp.asarray(area[:, None], dtype=jnp.float64)
    if senders.shape[0] == 0:
        relative = np.zeros((0, vertices.shape[1]), dtype=np.float64)
        distance = np.zeros((0, 1), dtype=np.float64)
    else:
    ...
    return MeshDualGraph(
        ...
        dual_edges=jnp.arange(senders.shape[0], dtype=jnp.int32),
