...
class OperatorTopology(StrictModule, NonTrainableState):
    ...
    def from_graph(
        ...
        *,
        ...
        if entity is None:
            resolved_entity = (
                "edge" if site == "edge" else "global" if site == "global" else "node"
            )
        else:
    ...
    def from_cochain(
        ...
        *,
        ...
        if sample_cells is None:
            ...
            if local_cells.ndim != 1 or not np.issubdtype(local_cells.dtype, np.integer):
            ...
            if np.any(local_cells < 0) or np.any(
                ...
                raise ValueError(
                    "sample_cells contain out-of-range degree-local indices."
                )
    ...
    def validate(self) -> None:
        ...
        for case, (mapping, count, offset, graph_valid) in enumerate(
            ...
            if (
                entity_mask is not None
                and selected.size
                and not np.all(entity_mask[int(offset) + selected])
            ):
                raise ValueError(
...
def pad_operator_topology(topology: OperatorTopology, size: int, /) -> OperatorTopology:
    ...
    return topology.replace_sample_entities(
        jnp.pad(topology.sample_entities, padding, constant_values=-1)
    )
...
def stack_operator_topologies(
    ...
    if any(
        ...
        raise ValueError(
            "Topology kind, sample site, and entity must match when stacking."
        )
...
def broadcast_operator_topology(
    ...
    if source and (len(target) < len(source) or target[-len(source) :] != source):
        raise ValueError(
