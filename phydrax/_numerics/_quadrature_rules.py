...
_GAUSS_KRONROD_POSITIVE: dict[int, tuple[tuple[float, ...], tuple[float, ...]]] = {
    15: (
        (0.0, 0.20778495500789848, 0.4058451513773972, 0.5860872354676911,
         0.7415311855993945, 0.8648644233597691, 0.9491079123427585,
         0.9914553711208126),
        (0.20948214108472782, 0.20443294007529889, 0.19035057806478542,
         0.1690047266392679, 0.14065325971552592, 0.10479001032225019,
         0.06309209262997856, 0.022935322010529224),
    ),
    21: (
        (0.0, 0.14887433898163122, 0.2943928627014602, 0.4333953941292472,
         0.5627571346686047, 0.6794095682990244, 0.7808177265864169,
         0.8650633666889845, 0.9301574913557082, 0.9739065285171717,
         0.9956571630258081),
        (0.1494455540029169, 0.14773910490133849, 0.14277593857706009,
         0.13470921731147334, 0.12349197626206584, 0.10938715880229764,
         0.0931254545836976, 0.07503967481091996, 0.054755896574351995,
         0.032558162307964725, 0.011694638867371874),
    ),
    31: (
        (0.0, 0.1011420669187175, 0.20119409399743451, 0.29918000715316884,
         0.3941513470775634, 0.4850818636402397, 0.5709721726085388,
         0.650996741297417, 0.7244177313601701, 0.790418501442466,
         0.8482065834104272, 0.8972645323440819, 0.937273392400706,
    ...
    41: (
        (0.0, 0.07652652113349734, 0.15260546524092267, 0.22778585114164507,
        ...
        (0.07660071191799965, 0.07637786767208074, 0.07570449768455667,
         ...
         0.014626169256971253, 0.008600269855642943, 0.0030735837185205317),
    ),
    51: (
        (0.0, 0.06154448300568508, 0.1228646926107104, 0.1837189394210489,
         0.24386688372098844, 0.30308953893110785, 0.36117230580938786,
         0.4178853821930377, 0.473002731445715, 0.5263252843347191,
         0.577662930241223, 0.6268100990103174, 0.6735663684734684,
         0.7177664068130843, 0.7592592630373576, 0.7978737979985001,
         0.833442628760834, 0.8658470652932756, 0.8949919978782753,
         0.9207471152817016, 0.9429745712289743, 0.9616149864258425,
         0.9766639214595175, 0.9880357945340772, 0.9955569697904981,
         0.9992621049926098),
        (0.061580818067832936, 0.061471189871425316, 0.061128509717053046,
         0.06053945537604586, 0.05972034032417406, 0.058689680022394206,
         0.057437116361567835, 0.055950811220412316, 0.05425112988854549,
         0.05236288580640747, 0.05027767908071567, 0.04798253713883671,
         0.04550291304992179, 0.04287284502017005, 0.04008382550403238,
         0.03711627148341554, 0.034002130274329335, 0.030792300167387487,
         0.02747531758785174, 0.024009945606953215, 0.020435371145882834,
         0.0168478177091283, 0.013236229195571676, 0.009473973386174152,
...
def clenshaw_curtis_data(order: int) -> QuadratureRuleData:
    ...
    order_ = int(order)
    if order_ < 2:
        raise ValueError("Clenshaw--Curtis order must be at least two.")
    n = order_ - 1
    ...
    return QuadratureRuleData(
        jnp.asarray(nodes), jnp.asarray(weights), None, order_ - 1
    )
...
def tanh_sinh_data(order: int) -> QuadratureRuleData:
    ...
    return QuadratureRuleData(
        jnp.asarray(nodes), jnp.asarray(weights), None, None
    )
