...
def _phase_shift_coefficients(
    ...
    for axis, size in zip(axes, source_shape, strict=True):
        if size % 2 == 0:
            result = _resize_fourier_axis(result, axis, size + 1)

    for local_axis, (axis, offset) in enumerate(zip(axes, offsets_, strict=True)):
        ...
        offset_ = jnp.asarray(raw_offset, dtype=result.real.dtype)
        offset_ = eqx.error_if(
            offset_, ~jnp.isfinite(offset_), "Fourier phase offsets must be finite."
        )
        size = int(result.shape[axis])
        modes = jnp.fft.fftfreq(size).astype(result.real.dtype) * size
        angle = 2.0 * jnp.asarray(jnp.pi, dtype=result.real.dtype) * modes * offset_
...
def fourier_resample(
    ...
    *,
    ...
    ``phase_offsets`` shifts each target grid by a fraction of its axis period.
    When ``axes`` is omitted, spatial axes are the trailing axes immediately
    before one channel/payload axis, matching Phydrax neural-operator layouts.
    """
    ...
    if phase_offsets is not None:
        coefficients = _phase_shift_coefficients(
            coefficients,
            resolved_axes,
            source_shape,
            phase_offsets,
