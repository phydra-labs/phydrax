...
class DimensionalScaleContract(StrictModule, NonTrainableState):
    """Shared length, mass, time, and coordinate-kind identity."""
    ...
    def __init__(
        ...
        *,
        ...
        if (
            ...
            or not mass
            or not time
            or kind not in ("physical", "comoving", "code")
            or any(not np.isfinite(value) or value <= 0.0 for value in factors)
        ):
            raise ValueError("Dimensional scale contract is invalid.")
    ...
    def si(cls) -> DimensionalScaleContract:
        return cls("m", "kg", "s", length_coordinate_kind="physical")
