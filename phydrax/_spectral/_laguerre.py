...
class RadialLaguerrePlan(StrictModule, NonTrainableState):
    ...
    def __init__(
        ...
        *,
        ...
        if preparation_estimate > selected_limit:
            raise ValueError(
                f"Radial Laguerre preparation exceeds max_precompute_bytes; estimated {preparation_estimate} bytes."
            )

        degree = np.arange(selected_bandlimit, dtype=np.float64)
        ...
        dimensionless_nodes = np.asarray(dimensionless_nodes[order], dtype=np.float64)
        ...
        identity = np.eye(selected_bandlimit, dtype=np.float64)
        defect = float(np.max(np.abs(balanced_basis.T @ balanced_basis - identity)))
        tolerance = 256.0 * selected_bandlimit * np.finfo(np.float64).eps
        if defect > tolerance:
            raise RuntimeError(
    ...
    def execution_id(self) -> str:
        ...
        return canonical_fingerprint(
            {
                "kind": "radial-laguerre-execution",
    ...
    def analysis(self, values: ArrayLike, /) -> Array:
        ...
        scalar = array.ndim >= 1 and array.shape[-1] == self.radial_bandlimit
        if scalar:
            leading_shape = tuple(array.shape[:-1])
            fields = array.reshape((prod(leading_shape), self.radial_bandlimit))
        elif array.ndim >= 2 and array.shape[-2] == self.radial_bandlimit:
            ...
            raise ValueError(
                "Radial analysis expects (..., radial_bandlimit) or (..., radial_bandlimit, channels)."
            )
    ...
    def synthesis(self, coefficients: ArrayLike, /) -> Array:
        ...
        if scalar:
            ...
            raise ValueError(
                "Radial synthesis expects (..., radial_bandlimit) or (..., radial_bandlimit, channels)."
            )
