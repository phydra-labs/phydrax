...
def _evaluate_drift(
    ...
    *,
    ...
    if values.shape != flat_state.shape:
        raise ValueError(
            f"drift must return one state vector per point; got {values.shape}, expected {flat_state.shape}."
        )
    values = eqx.error_if(
        values,
...
def _apply_diffusion(
    ...
    *,
    ...
    if callable(diffusion) or isinstance(diffusion, DomainFunction):
        ...
        coefficient = jnp.asarray(diffusion, dtype=jnp.float64)
...
def diffusion_paths_from_noise(
    ...
    *,
    ...
    start = jnp.broadcast_to(start, batch_shape + (start.shape[-1],))
    z = jnp.broadcast_to(
        z,
        batch_shape + (z.shape[-3], slicing.num_steps, z.shape[-1]),
    )
    z = eqx.error_if(z, ~jnp.all(jnp.isfinite(z)), "noise must be finite.")
    num_paths = z.shape[-3]
    state = jnp.broadcast_to(
        start[..., None, :],
        batch_shape + (num_paths, start.shape[-1]),
    )
...
def sample_diffusion_paths(
    ...
    *,
    ...
    noise = jr.normal(
        noise_key,
        start.shape[:-1] + (count, slicing.num_steps, start.shape[-1]),
        dtype=start.dtype,
    )
