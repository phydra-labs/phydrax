...
class HydrodynamicPhoreticSolver:
    ...
    def create(
        ...
        if radii.ndim != 1 or radii.size == 0 or np.any(radii <= 0):
            raise ValueError("Phoretic particle radii must be a positive vector.")
    ...
    def mobility(self, position_m: ArrayLike, /) -> tuple[Array, Array]:
        ...
        for left in range(count):
            ...
            for right in range(left + 1, count):
                ...
                direction = delta / jnp.maximum(distance, jnp.finfo(distance.dtype).tiny)
    ...
    def advance(
        ...
        if step_size_s <= 0:
            raise ValueError("Phoretic step size must be positive.")
        mobility, minimum_gap = self.mobility(positions)
        if bool(minimum_gap <= 0):
            raise ValueError(
                "Point-force hydrodynamics requires non-overlapping particles."
            )
