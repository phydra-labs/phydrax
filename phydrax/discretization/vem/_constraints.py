...
class VirtualElementDirichletConstraint(StrictModule, NonTrainableState):
    ...
    def lift(self, values: ArrayLike | Callable[[Array], ArrayLike], /) -> Array:
        ...
        if raw.shape == ():
            constrained_values = (
                jnp.where(self.trace_modes == 0, raw, 0.0)
                if self.trace_kind in ("normal", "tangential")
                else jnp.broadcast_to(raw, self.constrained_dofs.shape)
            )
...
def virtual_element_dirichlet_constraint(
    ...
    *,
    ...
    if discretization.field.element.family == "ConformingH1":
    ...
    return VirtualElementDirichletConstraint(
        ...
        trace_modes=jnp.asarray(trace_modes),
