...
class ColorGradientLBMMethod(StrictModule, NonTrainableState):
    """Conservative binary recoloring layered over one forced LBM method."""

    hydrodynamic_method: LatticeBoltzmannMethodPlan
    recoloring_strength: float = eqx.field(static=True)
    ...
    def __init__(
        ...
        *,
        ...
        if hydrodynamic_method.forcing is None:
            raise ValueError("Color-gradient CSF requires a forced LBM method.")
        values = tuple(
            float(value)
            for value in (
                recoloring_strength,
...
def recolor_populations(
    ...
    *,
    ...
    including momentum, is unchanged by recoloring.
    """
    ...
    if beta.shape != () or floor.shape != ():
        raise ValueError("Recoloring coefficients must be scalar.")
    beta = eqx.error_if(
        beta,
    ...
    return ColorGradientLBMState(red_populations, blue_populations)
...
class PreparedColorGradientLBMDynamics(StrictModule, NonTrainableState):
    ...
    def _fields(
        ...
        color = (red_density - blue_density) / safe_density
    ...
    def initialize_state(
        ...
        color = (red_lattice - blue_lattice) / safe_density
        ...
        initial = recolor_populations(
            ...
            interfacial.normal,
            self.discretization.velocity_set,
            self.method.recoloring_strength,
        ...
        return ColorGradientLBMState(red_checked, state.blue_populations)
    ...
    def _recoloring_conservation(
        self,
        recolored: ColorGradientLBMState,
        total: Array,
        red_density: Array,
        blue_density: Array,
        /,
    ) -> RecoloringConservation:
        red_moment = jnp.sum(recolored.red_populations, axis=-1)
        blue_moment = jnp.sum(recolored.blue_populations, axis=-1)
        closure = recolored.red_populations + recolored.blue_populations - total
    ...
    def scalar_diagnostics(
        ...
        conservation = RecoloringConservation(zero, zero, zero, zero)
    ...
    def step_detailed(
        ...
        conservation = self._recoloring_conservation(
            recolored, post_collision, target_red, target_blue
        )
        ...
        red_candidate = self.boundary.route(
            self.discretization.precision.population(recolored.red_populations),
            target_red,
            lattice_wall,
        )
        blue_candidate = self.boundary.route(
            self.discretization.precision.population(recolored.blue_populations),
            target_blue,
            lattice_wall,
        )
        ...
        accepted = ColorGradientLBMState(
