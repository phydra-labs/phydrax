...
def solve_event_driven_quantum_jump(
    ...
    *,
    ...
    """Bracket norm-threshold events and refine each crossing by bisection."""
    ...
    threshold_key, channel_key = jax.random.split(key)
    threshold = jax.random.uniform(threshold_key)
    ...
    for index in range(count):
        ...
        if bool(jax.device_get(crossing)) and event_count < event_capacity:
            lower = jnp.asarray(0.0, dtype=step.dtype)
            upper = step
            event_state = candidate
            for _ in range(int(bisection_iterations)):
                midpoint = 0.5 * (lower + upper)
                midpoint_state = _rk4(problem, start, midpoint)
                survival = jnp.real(jnp.vdot(midpoint_state, midpoint_state))
                if bool(jax.device_get(survival > threshold)):
                    lower = midpoint
                else:
                    upper = midpoint
                    event_state = midpoint_state
            ...
            local_key = jax.random.fold_in(channel_key, event_count)
            ...
            event_channels = event_channels.at[event_count].set(channel)
            ...
            event_count += 1
            threshold = jax.random.uniform(jax.random.fold_in(threshold_key, event_count))
        else:
