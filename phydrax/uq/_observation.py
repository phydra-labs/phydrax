...
from ..stochastic._state_space import AbstractObservationModel
...
class LikelihoodObservationModel(AbstractObservationModel):
    ...
    parameters_fn: Callable[[Array, Array], Mapping[str, Any]] | None
    ...
    def __init__(
        ...
        *,
        ...
        observation_id: str,
        parameters: Callable[[Array, Array], Mapping[str, Any]] | None = None,
    ):
    ...
    def _parameters(self, state: Array, time: Array, /) -> Mapping[str, Any]:
        return {} if self.parameters_fn is None else self.parameters_fn(state, time)

    def location(self, state: ArrayLike, time: ArrayLike, /) -> Array:
        state_array = jnp.asarray(state)
        values = jnp.asarray(self.location_fn(state_array, jnp.asarray(time)))
        if self.observation_shape and tuple(values.shape[-len(self.observation_shape) :]) != self.observation_shape:
    ...
    def log_prob(
        ...
        location = self.location(state_array, time_array)
        ...
        terms = jnp.asarray(
            self.likelihood.log_prob(
                location,
                target,
                **self._parameters(state_array, time_array),
            )
        )
    ...
    def sample(
        ...
        time_array = jnp.asarray(time)
        location = self.location(state_array, time_array)
        parameters = self._parameters(state_array, time_array)
        ...
        values = jax.vmap(
            lambda sample_key: self.likelihood.sample(
                sample_key, location, **parameters
            )
        )(keys)
