...
def _regularized_geometry(
    ...
    covariance = jnp.einsum("...ik,...k,...jk->...ij", vectors, values, jnp.conj(vectors))
...
def _covariance_structure(
    ...
    tied = jnp.einsum("...k,...kij->...ij", mass, covariance) / total[..., None, None]
...
def _fit_gaussian_mixture(
    ...
    *,
    ...
    def em_step(_, state):
        ...
        empirical_means = (
            jnp.einsum("...nk,...nf->...kf", weighted, x) / safe_mass[..., :, None]
        )
