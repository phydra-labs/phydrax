...
def _component_degree(component: DomainComponent, /) -> int:
    ...
    if len(selected) != 1:
        raise ValueError(
            "CochainResidualConstraint requires exactly one CochainCells selector."
        )
    return selected[0]
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(component, DomainComponent):
            raise TypeError("CochainResidualConstraint requires one DomainComponent.")
        if component.where or component.where_all is not None or component.weight_all is not None:
            raise ValueError(
                "CochainResidualConstraint does not accept component filters or weights; "
                "encode masks in CochainCells and scalar weighting in the constraint."
            )
    ...
    def loss(
        ...
        **kwargs: Any,
        ...
        if not isinstance(selected, GraphBatch):
            raise TypeError("CochainResidualConstraint requires a GraphBatch.")
...
__all__ = ["CochainResidualConstraint", "cochain_residual_field"]
