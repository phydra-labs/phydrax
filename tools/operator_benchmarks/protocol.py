...
class OperatorBenchmarkProtocol:
    """Compute, capacity, normalization, and search policy for benchmark protocol."""
...
def audit_geometry_scenario(
    ...
    *,
    ...
    if not required_inputs.issubset(scenario.train_batch.inputs):
        raise ValueError(
            "Geometry scenario audits require forcing, diffusivity, boundary_value, and boundary_indicator inputs."
        )
            ...
            if values is not None:
                ...
                if name == self.domain_support_key:
                    ...
                    source_total = eqx.error_if(
                        ...
                        "Conservation-aware normalization requires positive source measure in every case.",
                    )
...
def _target_parameters(
    ...
    if not common_minimum <= target <= common_maximum:
        raise ValueError(
            f"Capacity target {target} is outside the common feasible range [{common_minimum}, {common_maximum}]."
        )
    return target
            ...
            if candidate_metrics is not None:
                for other in raw_points:
                    ...
                    if all(
                        ...
                        dominated_by.append(
                            f"{other_comparison.architecture}@{other_comparison.size_scale:g}"
                        )
...
def _sample_efficiency_curve(
    ...
    *,
    ...
    x_values = np.asarray(fractions, dtype="float64")
    y_values = np.asarray(relative_l2, dtype="float64")
    if len(fractions) == 1:
    ...
    *,
    ...
    if selected_names is not None:
        ...
        if unavailable_names:
            ...
            raise ValueError(
                f"Requested architectures are incompatible with every selected scenario: {unavailable}."
            )
...
def save_benchmark_protocol_artifacts(
    directory: str | Path,
    ...
    aggregate_path = root / "operator_benchmarks.parquet"
    trial_path = root / "operator_benchmark_trials.parquet"
    sample_efficiency_path = root / "operator_sample_efficiency.parquet"
    promotion_path = root / "operator_promotions.parquet"
    symmetry_path = root / "operator_symmetry.parquet"
    portfolio_path = root / "operator_portfolio_promotions.parquet"
    difficulty_path = root / "operator_scenario_difficulty.parquet"
    pareto_path = root / "operator_pareto_fronts.parquet"
