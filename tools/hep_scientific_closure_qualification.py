...
def qualification_record():
    ...
    gates = tuple(
        phx.qualification.ReleaseGateEvidence(
            gate,
            passed=True,
            evidence_ids=(f"{gate}-evidence",),
            reviewer_id="external-review-required",
            issued_at=1,
            expires_at=2,
        )
        for gate in phx.particle_physics.HEP_RELEASE_GATES
    )
