...
def qualify(sample_count: int) -> dict[str, object]:
    ...
    reference_passive = (np.exp(4.0 * (fiber_length - 1.0) / 0.6) - 1.0) / np.expm1(4.0)
    ...
    tendon_roundtrip = np.asarray(
        de_groote_fregly_2016_inverse_tendon_force_length(parameters, observed_tendon)
    )
    ...
    errors = {
        ...
        "passive_curve_max_abs": float(
            np.max(np.abs(observed_passive - reference_passive))
        ),
        "force_velocity_max_abs": float(
            np.max(np.abs(observed_velocity - reference_velocity))
        ),
        "tendon_inverse_max_abs": float(np.max(np.abs(tendon_roundtrip - tendon_length))),
        "force_velocity_inverse_max_abs": float(
            np.max(np.abs(velocity_roundtrip - fiber_velocity))
        ),
        ...
        "tendon_constitutive_residual_max_abs": float(
            np.max(np.abs(equilibrium.evidence.tendon_constitutive_residual_normalized))
        ),
        "force_velocity_inverse_residual_max_abs": float(
            np.max(
                np.abs(equilibrium.evidence.force_velocity_inverse_residual_normalized)
            )
        ),
