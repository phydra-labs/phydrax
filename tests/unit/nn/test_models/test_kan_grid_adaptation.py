...
def _spline_kan(*, scan=False, use_tanh=False, key=jax.random.key(0)):
    return phx.nn.models.KAN(
        ...
        width_size=4,
        depth=3,
        edge_basis=phx.nn.models.BSplineEdgeBasis(degree=3, num_intervals=6),
        use_tanh=use_tanh,
...
def test_shared_grid_adaptation_is_pure_and_fixed_count():
    ...
    adapted, report = phx.nn.models.adapt_kan_grids(
        model,
        calibration,
        plan=phx.nn.models.KANGridAdaptationPlan(blend=0.05, minimum_span=1e-3),
    )
...
def test_per_input_adaptation_separates_marginal_distributions():
    model = phx.nn.models.KAN(
        ...
        out_size="scalar",
        hidden_sizes=(),
        edge_basis=phx.nn.models.BSplineEdgeBasis(degree=3, num_intervals=5),
        skip_connection=False,
    ...
    assert isinstance(bank, phx.nn.models.BSplineGridBank)
...
def test_adaptation_uses_the_layer_canonical_edge_inputs():
    model = phx.nn.models.KAN(
        ...
        out_size="scalar",
        hidden_sizes=(),
        edge_basis=phx.nn.models.BSplineEdgeBasis(degree=2, num_intervals=4),
        scale_mode="edge",
        use_tanh=True,
        skip_connection=False,
...
def test_degenerate_calibration_policy_and_failures_are_explicit():
    ...
    retained, retained_report = phx.nn.models.adapt_kan_grids(model, calibration)
    uniform, uniform_report = phx.nn.models.adapt_kan_grids(
        model,
        calibration,
        plan=phx.nn.models.KANGridAdaptationPlan(degenerate_policy="uniform"),
    )
    ...
    with pytest.raises(ValueError, match="finite"):
        phx.nn.models.adapt_kan_grids(model, jnp.asarray([[jnp.nan, 0.0]]))
...
def test_separable_kan_adapts_every_coordinate_model():
    ...
    adapted, report = phx.nn.models.adapt_kan_grids(model, calibration)
