...
def test_external_operator_manifest_roundtrip_and_adapter(tmp_path):
    ...
    assert jnp.allclose(compiled(adapter, batch), 2.0 * jnp.arange(5.0))
    ...
    assert verified.binding.binding_id == manifest.binding_identity().binding_id
    assert verified.model_execution_contract().execution.tier == "functional-jax"
