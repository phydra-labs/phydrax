...
def test_sourcewise_normalization_preserves_support_and_physical_mass(
    ...
    normalized = benchmark_protocol._NormalizedOperator(
    ...
    support_passthrough = benchmark_protocol._NormalizedOperator(
        SupportProjection(),
        statistics,
        0.0,
        1.0,
...
def test_runs_matched_search_and_persists_artifacts(
    ...
    monkeypatch.setattr(benchmark_protocol, "evaluate_operator", count_evaluation)
    ...
    paths = save_benchmark_protocol_artifacts(tmp_path / comparison, result)
    assert len(paths) == 9
...
def test_symmetry_benchmark_records_fno_defects_and_durable_artifact(tmp_path):
    ...
    hard = benchmark_protocol._tag_level(
        square_diffusion_symmetry_scenario(
            resolution=9,
            num_cases=6,
            chiral_strength=1e-5,
    ...
    ladder = benchmark_protocol.OperatorBenchmarkLadder(
        "focused_square_symmetry",
    ...
    paths = save_benchmark_protocol_artifacts(tmp_path, result)
    symmetry_path = next(
        path for path in paths if path.name == "operator_symmetry.parquet"
    )
    ...
    sample_path = next(
        path for path in paths if path.name == "operator_sample_efficiency.parquet"
    )
    ...
    assert all(path.name != "operator_symmetry_decisions.parquet" for path in paths)
    payload = json.loads(paths[0].read_text(encoding="utf-8"))
