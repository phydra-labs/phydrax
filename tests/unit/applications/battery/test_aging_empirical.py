...
def _support(
    *,
    ...
    return EmpiricalAgingSupport(
        ...
        source_id="test:empirical-aging-support",
    )
...
def _coefficients(**overrides):
    ...
    return EmpiricalAgingCoefficients(
...
def test_rest_has_calendar_exposure_but_signed_current_adds_equal_throughput():
    ...
    rest = advance_empirical_aging(
        topology,
        coefficients,
        state,
        _history(current=(0.0, 0.0, 0.0)),
    )
    charge = advance_empirical_aging(topology, coefficients, state, _history())
    discharge = advance_empirical_aging(
        topology,
        coefficients,
        state,
...
def test_positive_declared_coefficients_give_monotone_one_way_outputs():
    ...
    first = advance_empirical_aging(
        topology, coefficients, initial_empirical_aging_state(), _history()
    )
...
def test_invalid_latent_state_and_history_shape_fail_explicitly():
    ...
    assert result.status == int(EmpiricalAgingStatus.INVALID_STATE)
...
def test_jit_vmap_and_gradients_cover_coefficients_and_stress_inputs():
    ...
    compiled = jax.jit(advance_empirical_aging)(topology, coefficients, state, history)
    ...
    def evaluate_current(current):
        ...
        return advance_empirical_aging(
    ...
    def objective(scales, current):
        ...
        outputs = advance_empirical_aging(topology, varied, state, stress).observables
