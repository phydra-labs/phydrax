...
def _multispecies_state(system, cells):
    x = (jnp.arange(cells) + 0.5) / cells
    rho_a = 0.7 + 0.1 * jnp.sin(2.0 * jnp.pi * x)
    rho_b = 0.3 + 0.05 * jnp.cos(2.0 * jnp.pi * x)
    density = rho_a + rho_b
    velocity = 0.2 * jnp.ones_like(x)
    pressure = 1.0 + 0.1 * jnp.sin(4.0 * jnp.pi * x)
    provisional = jnp.stack(
        (rho_a, rho_b, density * velocity, jnp.ones_like(x)),
        axis=-1,
    )
    gamma = system.mixture_gamma(provisional)
    energy = pressure / (gamma - 1.0) + 0.5 * density * velocity**2
    return jnp.stack((rho_a, rho_b, density * velocity, energy), axis=-1)


def test_multispecies_euler_preserves_each_species_mass_and_admissibility():
    cells = 96
    system = phx.discretization.MultispeciesEuler1DSystem((1.4, 1.67))
    dynamics = phx.discretization.MultispeciesEuler1DDynamics(
        system,
        phx.discretization.HighResolutionReconstructionPlan("weno_z"),
        1.0 / cells,
    )
    state = _multispecies_state(system, cells)

    derivative = dynamics(0.0, state)
    updated = state + 1e-4 * derivative

    np.testing.assert_allclose(jnp.sum(derivative[:, :2], axis=0), 0.0, atol=2e-12)
    assert jnp.all(system.admissible(updated))


def test_ideal_mhd_normal_field_constraint_and_positive_state_survive_flux_update():
    cells = 96
    system = phx.discretization.IdealMHD1DSystem()
    x = (jnp.arange(cells) + 0.5) / cells
    density = 1.0 + 0.1 * jnp.sin(2.0 * jnp.pi * x)
    velocity = jnp.stack(
        (0.1 * jnp.ones_like(x), 0.02 * jnp.sin(2.0 * jnp.pi * x), jnp.zeros_like(x)),
        axis=-1,
    )
    magnetic = jnp.stack(
        (0.75 * jnp.ones_like(x), 0.1 * jnp.cos(2.0 * jnp.pi * x), jnp.zeros_like(x)),
        axis=-1,
    )
    pressure = jnp.ones_like(x)
    energy = (
        pressure / (system.gamma - 1.0)
        + 0.5 * density * jnp.sum(velocity**2, axis=-1)
        + 0.5 * jnp.sum(magnetic**2, axis=-1)
    )
    state = jnp.concatenate(
        (density[:, None], density[:, None] * velocity, energy[:, None], magnetic),
        axis=-1,
    )
    dynamics = phx.discretization.IdealMHD1DDynamics(
        system,
        phx.discretization.HighResolutionReconstructionPlan("teno"),
        1.0 / cells,
    )

    derivative = dynamics(0.0, state)
    updated = state + 1e-5 * derivative

    np.testing.assert_allclose(derivative[:, 5], 0.0, atol=0.0)
    assert jnp.all(system.admissible(updated))


def test_unsplit_multidimensional_flux_uses_shared_faces_and_preserves_mass():
    shape = (32, 24)
    x = (jnp.arange(shape[0]) + 0.5) / shape[0]
    y = (jnp.arange(shape[1]) + 0.5) / shape[1]
    state = (
        jnp.sin(2.0 * jnp.pi * x)[:, None] + 0.3 * jnp.cos(2.0 * jnp.pi * y)[None, :]
    )[..., None]
    velocities = (0.7, -0.2)
    dynamics = phx.discretization.UnsplitFluxDifferenceDynamics(
        (
            phx.discretization.HighResolutionReconstructionPlan("weno_z"),
            phx.discretization.HighResolutionReconstructionPlan("weno_z"),
        ),
        lambda values, axis, args: velocities[axis] * values,
        lambda left, right, axis, args: jnp.full(left.shape[:-1], abs(velocities[axis])),
        (1.0 / shape[0], 1.0 / shape[1]),
    )

    derivative = dynamics(0.0, state)

    np.testing.assert_allclose(jnp.sum(derivative), 0.0, atol=2e-10)
